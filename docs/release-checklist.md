...
- [x] **Telegram Reminders (TG-10):** El bot crea reminders con `channel: 'push'` y pregunta por hora exacta si falta (`needs_time`). (Telegram ya crea recordatorios push automáticos validados en dispositivo móvil real).
- [ ] **Google Calendar OAuth (CAL-2):** Tablas y Edge Functions de autenticación creadas. En preparación / pendiente de credenciales.
